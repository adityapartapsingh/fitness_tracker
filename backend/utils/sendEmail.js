...
const sendEmail = async (email, otpOrOptions) => {
  try {
    ...
    // 2. Handle Custom Options Object
    else {
      subject = otpOrOptions.subject || 'Fitness Tracker Notification';
      htmlContent = otpOrOptions.html || otpOrOptions.text;
    }

    const { data, error } = await resend.emails.send({
      from: 'Fitness Tracker <onboarding@resend.dev>', // Use your verified domain in production
      to: email,
      subject: subject,
      html: htmlContent,
    });
